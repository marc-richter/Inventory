...
@router.get("/sizes", response_model=schemas.SizesUpdate)
def my_sizes(db: Session = Depends(get_db), user: models.User = Depends(security.get_current_user)):
    p = db.get(models.Person, user.person_id) if user.person_id else None
    return schemas.SizesUpdate(sizes=(p.sizes or {}) if p else {})


@router.post("/sizes", response_model=schemas.SizesUpdate)
def set_my_sizes(payload: schemas.SizesUpdate, db: Session = Depends(get_db),
                 user: models.User = Depends(security.get_current_user)):
    """Eigene Groessen im Personenprofil hinterlegen (Konto muss mit einer Person
    verknuepft sein). Werte als Map {size_field_id: Wert}."""
    if not user.person_id:
        raise HTTPException(status_code=400, detail="Kein Personenprofil mit diesem Konto verknüpft")
    p = db.get(models.Person, user.person_id)
    if not p:
        raise HTTPException(status_code=404, detail="Person nicht gefunden")
    p.sizes = {str(k): str(v) for k, v in (payload.sizes or {}).items() if str(v).strip()}
    db.commit()
    log_action(db, user, "set_my_sizes", "person", p.id)
    return schemas.SizesUpdate(sizes=p.sizes)
