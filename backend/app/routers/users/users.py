...
def set_revoked_capabilities(user_id: int, payload: schemas.RevokedCapabilities,
    ...
    from app.permissions import CAP_KEYS
    u = db.get(models.User, user_id)
    if not u:
        raise HTTPException(status_code=404, detail="Benutzer nicht gefunden")
...
def merge_users(payload: schemas.MergeUsersRequest, db: Session = Depends(get_db),
    ...
    src = db.get(models.User, payload.source_id)
    tgt = db.get(models.User, payload.target_id)
    ...
    if src.person_id:
        if tgt.person_id and tgt.person_id != src.person_id:
            ...
            srcp = db.get(models.Person, src.person_id)
            if srcp:
                srcp.active = False
