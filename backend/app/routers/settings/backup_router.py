...
from app.backup import create_backup, restore_backup, _backup_dir, verify_backup
...
def download_backup(backup_id: int, db: Session = Depends(get_db), user=Depends(security.require_roles("admin"))):
    rec = db.get(models.BackupRecord, backup_id)
    if not rec:
...
def verify_backup_endpoint(backup_id: int, db: Session = Depends(get_db), user=Depends(security.require_roles("admin"))):
    """Prueft ein Backup auf Vollstaendigkeit und Integritaet (ZIP, SQLite, Schema)."""
    rec = db.get(models.BackupRecord, backup_id)
    if not rec:
        raise HTTPException(status_code=404, detail="Backup nicht gefunden")
    path = _backup_dir(db) / rec.filename
    ...
    log_action(db, user, "verify_backup", "backup", rec.id, {"ok": result["ok"]})
    return result


@router.post("/verify-upload")
async def verify_uploaded_backup(file: UploadFile = File(...), db: Session = Depends(get_db),
                                  user=Depends(security.require_roles("admin"))):
    """Prueft ein hochgeladenes Backup-Zip ohne es zu importieren."""
    tmp_path = DATA_DIR / f"_verify_upload_{file.filename}"
    content = await file.read()
    tmp_path.write_bytes(content)
    try:
        ...
        tmp_path.unlink(missing_ok=True)
    log_action(db, user, "verify_backup_upload", "backup", None, {"ok": result["ok"], "filename": file.filename})
    return result
