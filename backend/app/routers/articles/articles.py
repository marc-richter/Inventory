...
@router.get("", response_model=schemas.ArticlePaginatedOut)
def list_articles(
...
def article_revision(article_id: int, db: Session = Depends(get_db),
    ...
    if last and last.user_id:
        u = db.get(models.User, last.user_id)
        if u:
...
def article_history(article_id: int, db: Session = Depends(get_db),
    ...
    if not db.get(models.Article, article_id):
        raise HTTPException(status_code=404, detail="Artikel nicht gefunden")
...
def create_article(payload: schemas.ArticleCreate, db: Session = Depends(get_db),
    ...
    if payload.model_id:
        m = db.get(models.ArticleModel, payload.model_id)
        if m:
            model_name = m.name
    a = models.Article(
        artikelnummer=artikelnummer,
...
def create_articles_bulk(payload: schemas.BulkArticleCreate, db: Session = Depends(get_db),
    ...
    _t = db.get(models.ArticleType, payload.type_id)
...
@router.delete("/bulk")
def bulk_delete_articles(payload: dict, db: Session = Depends(get_db),
                         user=Depends(security.require_capability("articles"))):
    """Mehrere Artikel auf einmal löschen. Erwartet {ids: [int, ...]}."""
    ids = payload.get("ids", [])
    if not ids:
        raise HTTPException(status_code=400, detail="Keine IDs übergeben")
    if len(ids) > 500:
        raise HTTPException(status_code=400, detail="Maximal 500 Artikel auf einmal")
    deleted = 0
    for aid in ids:
        ...
        if a:
            ...
            deleted += 1
    if deleted:
        log_action(db, user, "bulk_delete_articles", "article", None, {"ids": ids, "count": deleted})
        db.commit()
...
def bulk_update_status(payload: dict, db: Session = Depends(get_db),
                        user=Depends(security.require_capability("articles"))):
    """Status mehrerer Artikel auf einmal ändern. Erwartet {ids: [int, ...], status: str}."""
    ids = payload.get("ids", [])
    status = payload.get("status", "")
    if not ids:
        raise HTTPException(status_code=400, detail="Keine IDs übergeben")
    if not status:
        raise HTTPException(status_code=400, detail="Kein Status übergeben")
    if len(ids) > 500:
        raise HTTPException(status_code=400, detail="Maximal 500 Artikel auf einmal")
    updated = 0
    for aid in ids:
        ...
        if a:
            ...
            updated += 1
    if updated:
        log_action(db, user, "bulk_update_status", "article", None, {"ids": ids, "status": status, "count": updated})
        db.commit()
...
def delete_image(image_id: int, db: Session = Depends(get_db),
                  user=Depends(security.require_capability("articles"))):
    img = db.get(models.ArticleImage, image_id)
    if not img:
        raise HTTPException(status_code=404, detail="Bild nicht gefunden")
    if img.kind == "damage":
        raise HTTPException(
