...
def issue_article(payload: schemas.IssueCreate, db: Session = Depends(get_db),
    ...
    article = db.get(models.Article, payload.article_id)
    if not article:
...
def issue_batch(payload: schemas.BatchIssueRequest, db: Session = Depends(get_db),
    ...
    for item in payload.items:
        a = db.get(models.Article, item.article_id)
        if not a:
...
def return_article(issue_id: int, payload: schemas.ReturnCreate, db: Session = Depends(get_db),
                    user=Depends(security.require_capability("issues"))):
    rec = db.get(models.IssueRecord, issue_id)
    if not rec:
    ...
    article = db.get(models.Article, rec.article_id)
