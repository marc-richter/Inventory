...
def update_size_field(field_id: int, payload: schemas.SizeFieldUpdate, db: Session = Depends(get_db),
                      user=Depends(security.require_roles("admin", "verwalter"))):
    f = db.get(models.SizeField, field_id)
    if not f:
...
def update_model(model_id: int, payload: schemas.RenameRequest, db: Session = Depends(get_db),
                 user=Depends(security.require_roles("admin", "verwalter"))):
    m = db.get(models.ArticleModel, model_id)
    if not m:
...
def classify_storage_location(loc_id: int, payload: schemas.ClassifyStandortRequest,
    ...
    loc = db.get(models.StorageLocation, loc_id)
    if not loc:
    ...
    if payload.parent_standort_id:
        parent = db.get(models.StorageLocation, payload.parent_standort_id)
